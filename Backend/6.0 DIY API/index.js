...
app.get("/filter",(req,res)=>{
    ...
    const matchingJokes = jokes.filter((element) => (element.jokeType == type))
...
app.put("/jokes/:id",(req,res)=>{
    ...
    jokes.forEach((member) => {
        if(member.id == id){
            member.jokeText = text;
            member.jokeType = type;
            member.id = id;
        }
    })
    res.json(jokes[id-1]);
...
app.patch("/jokes/:id",(req,res)=>{
    const id = parseInt(req.params.id);
    const existingJoke = jokes.find((member) => member.id === id)
...
app.delete("/jokes/:id", (req,res) => {
    const id = parseInt(req.params.id);
    const indexToRemove = jokes.findIndex((member) => member.id === id)
    if(indexToRemove > -1){
...
app.delete("/all" , (req,res) => {
    if(req.query.key == masterKey){
        ...
        res.json({message : "all jokes were deleted"})
    }else{
