...
router.post("/register", async (req, res) => {
  try {
    ...
    if (error.code === 11000) { // MongoDB duplicate key error (e.g., if username was also unique)
      res.status(400).json({ message: 'Username or email is already taken.' });
    } else if (error.name === 'ValidationError') { // Mongoose validation errors (e.g., required field missing, minlength)
